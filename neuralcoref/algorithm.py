...
from neuralcoref.data import Data
from .config import GREEDYNESS, MAX_DIST, MAX_DIST_MATCH, MAX_FOLLOW_UP, \
    MENTION_TYPE, NO_COREF_LIST, SPACY_MODEL
...
class Model:
    ...
    def _score(self, features, layers):
        for W, bias in layers:
            '''
            tested scipy.linalg.blass and tensorflow.
            get minor (30%?) speedup by switching from np.matmul to 
            dot and another (2%?) speecup by specifying out parameter
            '''
            features = np.dot(W, features,
                              out=np.ndarray(shape=bias.shape, dtype=float)) + bias
...
class Coref:
    ...
    def __init__(self, nlp=None, spacy_model=SPACY_MODEL, greedyness=GREEDYNESS,
                 max_dist=MAX_DIST, max_dist_match=MAX_DIST_MATCH,
                 max_follow_up=MAX_FOLLOW_UP, conll=None, use_no_coref_list=True,
                 debug=False):
        ...
        print(f'PARAMS: greedyness: {greedyness}, max_dist: {max_dist}, '
        ...
        self.data = Data(nlp, model_path=embed_model_path, conll=conll,
                         use_no_coref_list=use_no_coref_list, consider_speakers=conll)
    ...
    def display_clusters(self):
        ...
        for key, mentions in list(self.clusters.items()):
            print("cluster", key, "(", ", ".join(
                str(self.data[m]) for m in mentions), ")")
    ...
    def run_coref_on_mentions(self, mentions):
        ...
        best_ant = {}
        n_ant = 0
        for mention_idx, ant_list in self.data.get_candidate_pairs(mentions,
                                                                   self.max_dist,
                                                                   self.max_dist_match):
            ...
            self.mentions_single_features[mention_idx] = {
                ...
                "wordsEmbeddings": mention.words_embeddings_, "features": feats_}
            ...
            if mention_idx in best_ant:
                n_ant += 1
                self._merge_coreference_clusters(
                    best_ant[mention_idx], mention_idx)
    ...
    def run_coref_on_utterances(self, last_utterances_added=False, follow_chains=True):
        ...
        mentions = list(self.data.get_candidate_mentions(
    ...
    def one_shot_coref(self, utterances, utterances_speakers_id=None, context=None,
        ...
        self.continuous_coref(
            utterances, utterances_speakers_id, speakers_names)
        return self.get_clusters()

    def continuous_coref(self, utterances, utterances_speakers_id=None,
                         speakers_names=None):
        '''
        ...
        Return:
            clusters of entities with coreference resolved
        '''
        self.data.add_utterances(
            utterances, utterances_speakers_id, speakers_names)
        self.run_coref_on_utterances(
            last_utterances_added=True, follow_chains=True)
        return self.get_clusters()
    ...
    def get_resolved_utterances(self, last_utterances_added=True, use_no_coref_list=True):
        ...
        coreferences = self.get_most_representative(
            last_utterances_added, use_no_coref_list)
        resolved_utterances = []
        for utt in self.get_utterances(last_utterances_added=last_utterances_added):
            ...
            for token in utt:
                if in_coref is None:
                    for coref_original, coref_replace in list(coreferences.items()):
    ...
    def get_most_representative(self, last_utterances_added=True, use_no_coref_list=True):
        ...
        clusters = self.get_clusters(
            remove_singletons=True, use_no_coref_list=use_no_coref_list)
        coreferences = {}
